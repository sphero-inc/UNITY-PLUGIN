...
public class CalibrateButton : MonoBehaviour {
	...
	void SetCalibrateButtonSize() {
		...
		Rect ringRect = new Rect(buttonMid.x-ringSize*0.5f,buttonMid.y,ringSize,ringSize);
		outerRing.pixelInset = ringRect;  
		float midSize = ringSize*0.88235f;
		middleRing.pixelInset = new Rect(buttonMid.x-midSize*0.5f,buttonMid.y+(ringSize-midSize)*0.5f,midSize,midSize);
		float innerSize = ringSize*0.9f;
		innerRing.pixelInset = new Rect(buttonMid.x-innerSize*0.5f,buttonMid.y+(ringSize-innerSize)*0.5f,innerSize,innerSize);
	...
	void SetRingsVisibility(bool visible) {
		innerRing.gameObject.active = visible;
		middleRing.gameObject.active = visible;	
		outerRing.gameObject.active = visible;
	}
